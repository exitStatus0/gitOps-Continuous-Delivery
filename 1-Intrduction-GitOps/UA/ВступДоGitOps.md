...
*Друга година ночі. Production-кластер лежить. Людина, яка востаннє його налаштовувала, звільнилася кілька місяців тому. Ніхто не знає, що змінилося, ніхто не може відтворити конфігурацію, а кожна хвилина простою коштує грошей. А тепер уявіть протилежне: усе середовище можна відновити з одного Git-репозиторію, маючи перед очима зрозумілу історію кожної зміни. Саме в цій різниці і є сенс GitOps.*

*У цьому й полягає обіцянка GitOps.*

![Огляд GitOps](../GitOpsDetailed.png)

## Що таке GitOps?

GitOps — це сучасна модель керування інфраструктурою та доставкою застосунків. Вона розглядає Git як єдине джерело істини для декларативних систем і покладається на програмні агенти, які приводять живе середовище у відповідність до того, що описано в репозиторії. Проєкт OpenGitOps формулює цей підхід через чотири фундаментальні принципи:
...
- **ArgoCD** - популярний GitOps оператор з потужним UI (розглядається в цьому курсі!)
...
**Готові перевірити свої знання? Спробуйте [тест з GitOps](перевірка-знань.md), щоб побачити, наскільки добре ви розумієте ці концепції!**

---

[&larr; Попередній: Вступ до Continuous Delivery](../../0-Introduction-CD/UA/ВступДоCD.md) | [Далі: Вступ до ArgoCD &rarr;](../../2-Introduction-to-ArgoCD/UA/ВступДоArgoCD.md)
