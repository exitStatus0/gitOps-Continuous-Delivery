...
В этом сценарии вы выступаете в роли разработчика приложений в **Forge World Sigma-VII**, работающего над новым сервисом **cogitator**. Ваша задача — развернуть этот сервис в Kubernetes, используя ArgoCD и практики GitOps.
...
Команда платформы в Forge World Sigma-VII недавно начала создание среды Kubernetes для размещения сервисов организации. Поскольку сервис cogitator является новым, у вашей команды есть прекрасная возможность протестировать среду Kubernetes.

Проблема заключается в том, что вы должны отказаться от старых методов развертывания сервисов, так как при переходе на Kubernetes организация внедряет **практики GitOps с использованием ArgoCD**.
...
Проверьте свое понимание с помощью этих вопросов:
