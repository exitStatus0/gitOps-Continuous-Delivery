...
**Application CRD (Custom Resource Definition)** — это ресурс, который фактически даёт ArgoCD инструкции к действию. Он декларативно определяет:
- откуда брать манифесты
- как их рендерить
- куда и когда развертывать ресурсы
- как действовать при согласовании живого состояния с желаемым
- и какие операционные правила должен учитывать ArgoCD

Когда вы понимаете этот ресурс, ArgoCD перестаёт выглядеть как чёрный ящик. Вместо этого появляется ясная связь между вашим намерением в Git и поведением контроллера в кластере.
